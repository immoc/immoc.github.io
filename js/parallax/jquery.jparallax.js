	...
	for (var i=0; i<layer.length; i++) {
		...
		if (layer[i].xparallax) {
			if (layer[i].xtravelpx) {
				...
				var mOffset = (layer[i].width-1583)/2;
				mOffset = layer[i].width - mOffset;
				//alert(mOffset + " " + layer[i].width);
				cssObject.marginLeft = xpos * mOffset *-1 + 'px';
			}
		}
...
jQuery.fn.jparallax.settings = {
	...
	frameDuration:      50				// In milliseconds
};
